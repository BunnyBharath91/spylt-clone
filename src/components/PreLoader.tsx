...
const PreLoader = ({ onComplete }: { onComplete: () => void }) => {
  ...
  useEffect(() => {
    ...
    // Count stylesheets with CSS backgrounds
    const styleSheets = document.styleSheets.length;
    
    // Include custom fonts and stylesheets in resource count
    const total = Math.max(resources.length + styleSheets, 1);
    let loaded = 0;
    ...
    // Track document ready state and stylesheets
    ...
    // Fallback: complete loading when window load event fires
    ...
    return () => {
      ...
      document.removeEventListener("readystatechange", handleWindowLoad);
    };
  }, []);
  ...
  return (
    <div className="preloader fixed inset-0 flex flex-col items-center justify-end pb-20 z-[9999] text-white bg-[#7f3b2d]">
      ...
      <img
        ...
        alt="pre img"
        className="lg:mb-40 mb-[60%] lg:w-[20%] w-[40%]"
      />
